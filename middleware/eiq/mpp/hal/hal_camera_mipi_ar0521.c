...
/*
 * @brief mipi_ar0521 camera module HAL camera driver implementation.
 */
...
#if (defined HAL_ENABLE_CAMERA) && (HAL_ENABLE_CAMERA_DEV_MipiAr0521 == 1)
#include <FreeRTOS.h>
#include <task.h>
#include <stdlib.h>
...
hal_camera_status_t HAL_CameraDev_MipiAr0521_Enqueue(const camera_dev_t *dev, void *data);
void HAL_CameraDev_MipiAr0521_ReceiverCallback(camera_receiver_handle_t *handle, status_t status, void *userData);
...
#define CAMERA_NAME "MipiAr0521"

/* TODO get those values from framework/user at 'init' */
#define CAMERA_DEV_MipiAr0521_BUFFER_COUNT 3
#define CAMERA_DEV_MipiAr0521_MAX_HEIGHT 720    /* set camera max resolution equal to display max */
#define CAMERA_DEV_MipiAr0521_MAX_WIDTH 1280    /* set camera max resolution equal to display max */
#define CAMERA_DEV_MipiAr0521_MAX_BPP 4             /* max value for YUYV */
#define CAMERA_DEV_MipiAr0521_ROTATE ROTATE_0
#define CAMERA_DEV_MipiAr0521_FLIP FLIP_NONE
#define CAMERA_DEV_MipiAr0521_SWAPBYTE 0
...
AT_NONCACHEABLE_SECTION_ALIGN(
    static uint8_t s_cameraBuffer[CAMERA_DEV_MipiAr0521_BUFFER_COUNT][CAMERA_DEV_MipiAr0521_MAX_HEIGHT]
                                 [CAMERA_DEV_MipiAr0521_MAX_WIDTH * CAMERA_DEV_MipiAr0521_MAX_BPP],
...
void MipiAr0521_InitCameraResource(void)
{
...
static status_t MipiAr0521_VerifyCameraClockSource(void)
{
    status_t status;
    uint32_t srcClkFreq;
    /*
...
void MipiAr0521_InitMipiCsi(mpp_camera_params_t *config)
{
    ...
    if (kStatus_Success != MipiAr0521_VerifyCameraClockSource())
    {
...
hal_camera_status_t HAL_CameraDev_MipiAr0521_Init(
    camera_dev_t *dev, mpp_camera_params_t *config, camera_dev_callback_t callback, void *param)
{
    ...
    HAL_LOGD("++HAL_CameraDev_MipiAr0521_Init( param[%p]\n", param);
    ...
    CAMERA_RECEIVER_Init(&cameraReceiver, &cameraConfig, HAL_CameraDev_MipiAr0521_ReceiverCallback, NULL);

    MipiAr0521_InitMipiCsi(config);
    ...
    for (uint32_t i = 0; i < CAMERA_DEV_MipiAr0521_BUFFER_COUNT; i++)
    {
        CAMERA_RECEIVER_SubmitEmptyBuffer(&cameraReceiver, (uint32_t)(s_cameraBuffer[i]));
    }

    HAL_LOGD("--HAL_CameraDev_MipiAr0521_Init\n");
    return ret;
}

hal_camera_status_t HAL_CameraDev_MipiAr0521_Getbufdesc(const camera_dev_t *dev, hw_buf_desc_t *out_buf, mpp_memory_policy_t *policy)
{
    hal_camera_status_t ret = kStatus_HAL_CameraSuccess;
    HAL_LOGD("++HAL_CameraDev_MipiAr0521_Getbufdesc(out_buf=[%p])\n", out_buf);
    do
    {
    ...
    HAL_LOGD("--HAL_CameraDev_MipiAr0521_Getbufdesc\n");
    return ret;
}

hal_camera_status_t HAL_CameraDev_MipiAr0521_Deinit(camera_dev_t *dev)
{
    hal_camera_status_t ret = kStatus_HAL_CameraSuccess;
    return ret;
}
...
hal_camera_status_t HAL_CameraDev_MipiAr0521_Start(const camera_dev_t *dev)
{
    hal_camera_status_t ret = kStatus_HAL_CameraSuccess;
    status_t status = kStatus_Success;
    HAL_LOGD("++HAL_CameraDev_MipiAr0521_Start\n");
...
hal_camera_status_t HAL_CameraDev_MipiAr0521_Stop(const camera_dev_t *dev)
{
    hal_camera_status_t ret = kStatus_HAL_CameraSuccess;
    status_t status = kStatus_Success;
    HAL_LOGD("++\n");
...
hal_camera_status_t HAL_CameraDev_MipiAr0521_Dequeue(const camera_dev_t *dev, void **data, mpp_pixel_format_t *format)
{
    hal_camera_status_t ret = kStatus_HAL_CameraSuccess;
    HAL_LOGD("++HAL_CameraDev_MipiAr0521_Dequeue\n");
...
const static camera_dev_operator_t camera_dev_mipi_ar0521_ops = {
    .init        = HAL_CameraDev_MipiAr0521_Init,
    .deinit      = HAL_CameraDev_MipiAr0521_Deinit,
    .start       = HAL_CameraDev_MipiAr0521_Start,
    .stop        = HAL_CameraDev_MipiAr0521_Stop,
    .enqueue     = HAL_CameraDev_MipiAr0521_Enqueue,
    .dequeue     = HAL_CameraDev_MipiAr0521_Dequeue,
    .get_buf_desc = HAL_CameraDev_MipiAr0521_Getbufdesc,
};

static camera_dev_t camera_dev_mipi_ar0521 = {
    .id   = 1,
    .name = CAMERA_NAME,
    .ops  = &camera_dev_mipi_ar0521_ops,
    .config =
        {
            .rotate   = CAMERA_DEV_MipiAr0521_ROTATE,
            .flip     = CAMERA_DEV_MipiAr0521_FLIP,
            .swapByte = CAMERA_DEV_MipiAr0521_SWAPBYTE,
        },
...
void HAL_CameraDev_MipiAr0521_ReceiverCallback(camera_receiver_handle_t *handle, status_t status, void *userData)
{
    if (camera_dev_mipi_ar0521.cap.callback != NULL)
    {
        uint8_t fromISR = __get_IPSR();
        camera_dev_mipi_ar0521.cap.callback(&camera_dev_mipi_ar0521, kCameraEvent_SendFrame,
                                            camera_dev_mipi_ar0521.cap.param, fromISR);
    }
}
...
{
    int error = 0;
    HAL_LOGD("++HAL_CameraDev_MipiAr0521_Enqueue\n");
    /* nothing to do, see HAL_CameraDev_MipiAr0521_Dequeue() */
    HAL_LOGD("--HAL_CameraDev_MipiAr0521_Enqueue\n");
    return error;
}

int HAL_CameraDev_MipiAr0521_setup(const char *name, camera_dev_t *dev, _Bool defconfig)
{
    dev->ops = &camera_dev_mipi_ar0521_ops;

    return 0;
}
#else /* (defined HAL_ENABLE_CAMERA) && (HAL_ENABLE_CAMERA_DEV_MipiAr0521 == 1) */
int HAL_CameraDev_MipiAr0521_setup(const char *name, camera_dev_t *dev, _Bool defconfig)
{
    HAL_LOGE("Camera MipiAr0521 not enabled\n");
    return -1;
}
#endif /* (defined HAL_ENABLE_CAMERA) && (HAL_ENABLE_CAMERA_DEV_MipiAr0521 == 1) */
